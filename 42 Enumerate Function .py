...
# (such as a list, tuple, or string) and get the index and value 
# of each element in the sequence at the same time. Here's a basic example of how it works:
...
# By default, the enumerate function starts the index at 0, but you can specify 
# a different starting index by passing it as an argument to the enumerate function:
...
# you might use it to loop over a list of strings and print the index and value of 
# each string in a formatted way:
...
marks = [20, 59, 90, 98, 50,60]
index = 0
for mark in marks:
    print(mark)
    if(index == 3):
        print("Rajesh Awesome")
    index +=1
...
for index, mark in enumerate(marks, start=1):     #index start with 1
    print(mark)
    if(index==3):
        print("Rajesh Awesome!!!!")